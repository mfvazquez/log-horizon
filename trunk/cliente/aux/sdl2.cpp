...
int main(void){
  ...
  // agregamos un fondo
  SDL_Surface *image = IMG_Load("fondo1_hda.png");
  ...
  image = IMG_Load("fondo3.jpg");
  SDL_Surface *fondo_celda = IMG_Load("imagenes/celda_fondo.png");
  int caca = SDL_BlitSurface(fondo_celda, NULL, image, NULL);
  if (caca == -1) std::cout << "caca" << std::endl;
  SDL_Texture *fondo2 = SDL_CreateTextureFromSurface(renderer, image);
  SDL_FreeSurface(image);
  SDL_FreeSurface(fondo_celda);
