...
class ListaFueraDeRango : public std::exception {};
...
U Lista<U>::borrarPrimero(){
    if (esVacia()) throw ListaFueraDeRango();
...
U Lista<U>::borrar(ListaIter<U>* iter){
    if (iter->alFinal()) throw ListaFueraDeRango();
...
W ListaIter<W>::verActual(){
    if (this->alFinal()) throw ListaFueraDeRango();
    return actual->dato;
}
